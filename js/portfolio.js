...
function fitLightboxImage() {
  if (!lb || !lbImg || !lbImg.naturalWidth || !lbImg.naturalHeight) return;

  const cs = window.getComputedStyle(lb);
  const padX = parseFloat(cs.paddingLeft) + parseFloat(cs.paddingRight);
  const padY = parseFloat(cs.paddingTop) + parseFloat(cs.paddingBottom);

  const availableWidth = Math.max(1, window.innerWidth - padX);
  const availableHeight = Math.max(1, window.innerHeight - padY);
  const ratio = lbImg.naturalWidth / lbImg.naturalHeight;
...
window.addEventListener('resize', () => {
  if (lb?.classList.contains('is-open')) fitLightboxImage();
}, { passive: true });
